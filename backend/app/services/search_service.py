...
class SearchService:
        ...
        if index_path.exists():
            try:
                ...
                if mapping_path.exists():
                    ...
                    logger.success(
                        f"Embedding ID mapping loaded — {len(self.embedding_id_mapping)} entries"
                    )
                else:
            ...
            logger.info("Command: python -m backend.pipelines.ingest.ingest --podcast <name>")
            self.index = None
    ...
    def search(
        ...
        Args:
            ...
            min_confidence: Optional minimum similarity threshold (0-1).
                If None, returns top_k results. If set, discards results below it.

        Returns:
            List of dicts with episode metadata and score.
        """
        ...
        if cached_result is not None:
            result_count = len(cached_result) if hasattr(cached_result, "__len__") else "?"
            logger.info(
                f"[CACHE] HIT  - query='{query}', top_k={top_k},"
                f" min_confidence={min_confidence}, results={result_count}"
            )
            return cached_result

        logger.info(
            f"[CACHE] MISS - query='{query}', top_k={top_k},"
        ...
        # Over-fetch FAISS candidates to compensate for:
        # 1. Post-filter attrition (podcast_source / program_name filters discard results)
        # 2. Deduplication (multiple segments per episode → only best is kept)
        has_filters = podcast_source is not None or program_name is not None
        multiplier = 20 if has_filters else 5
        search_k = min(top_k * multiplier, self.index.ntotal)
        ...
        try:
            for distance, faiss_idx in zip(distances[0], indices[0]):
                ...
                # IndexFlatIP on L2-normalized vectors = cosine similarity (0–1)
                ...
                program = (
                    episode_metadata.program_name
                    if episode_metadata and getattr(episode_metadata, "program_name", None)
                    else None
                )
                podcast_src = (
                    episode_metadata.podcast_source
                    if episode_metadata and getattr(episode_metadata, "podcast_source", None)
                    else None
                )
                author = (
                    program.strip() if program and program.strip()
                    else (podcast_src or None)
                )
        ...
        # Trim to top_k after deduplication
        ...
        logger.info(
            f"[CACHE] SET  - query='{query}', top_k={top_k},"
            f" min_confidence={min_confidence}, cached={len(results)}"
        )
