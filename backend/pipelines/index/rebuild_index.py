...
def rebuild_faiss_index():
    ...
    try:
        ...
        if dim_counts:
            ...
            if len(dim_counts) > 1:
                logger.warning(f"Mixed dimensions detected: {dim_counts}")
                logger.warning(f"Will only use {dimension_target}-dim embeddings")
        ...
        if skipped_no_emb > 0:
            logger.warning(f"Skipped {skipped_no_emb} segments without embeddings")
        if skipped_wrong_dim > 0:
            logger.warning(f"Skipped {skipped_wrong_dim} segments with wrong dimension (old model)")
