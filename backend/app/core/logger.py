"""
Centralized logging for Nerdcast Finder
Provides colorized console output with timestamps
"""
...
class Logger:
    """Colorized logger with timestamp support"""
    ...
    def progress(cls, current: int, total: int, item_name: Optional[str] = None):
        ...
        if item_name:
            msg = f"{progress_bar} {item_name}"
        else:
            msg = progress_bar
        print(f"{color}[{timestamp}] [PROGRESS] {msg}{cls.RESET}")
